...
export default {
  ...
  computed: {
    ...
    // 安全距离
    safeArea() {
      // const padArr = this.padding.split(" ").map(item => item * 100);
      // return padArr;
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      // 遵循此次项目的rem计算方式转换成px
      const remNum = 100 * (document.documentElement.clientWidth / 750);
      this.top = this.propTop * remNum; // rem转px;
      // const remNum = document.documentElement.clientWidth / 10;
      // this.top = (this.propTop / remNum) * remNum; // rem转px;
